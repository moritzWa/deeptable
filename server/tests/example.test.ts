...
describe("Best electric scooter", async () => {
  ...
  const cells = [
    {row: "Segway Ninebot Max G2", col: "weight (kg)", outputType: "number", expectedValue: "24.3"},
    {row: "Segway Ninebot Max G2", col: "maximum range (km)", outputType: "number", expectedValue: "70"},
    {row: "Segway Ninebot Max G2", col: "maximum speed (km/h)", outputType: "number", expectedValue: "35"}
  ];
...
async function extractThingy(query: string, row: string, col: string, outputType: string, searchResponses: Array<{response: string, provider: string}>) {
...
async function util(query: string, row: string, col: string, outputType: string) {
    ...
    const searchPromises = [
        doResearch(question).then(response => ({ response, provider: "OpenAI" }))
            .catch(err => ({ response: `Error with OpenAI search: ${err.message}`, provider: "OpenAI" })),
        doPerplexity(question).then(response => ({ response, provider: "Perplexity" }))
            .catch(err => ({ response: `Error with Perplexity search: ${err.message}`, provider: "Perplexity" })),
        doGoogle(question).then(response => ({ response, provider: "Google" }))
            .catch(err => ({ response: `Error with Google search: ${err.message}`, provider: "Google" }))
    ];
